...
class RemoteSandboxBackend(SandboxBackend):
    ...
    def _build_mount_command(
        self, *, workspace_id: str, skill_scope: str | None
    ) -> str:
        ...
        if missing:
            raise RuntimeError(
                f"OSS mount configuration is incomplete: missing {', '.join(missing)}"
            )
        ...
        return (
            f"printf '%s' {self._quote_shell(encoded_script)} | base64 -d | /bin/bash"
        )
    ...
    def _provisioner_create(
        ...
        try:
            ...
            logger.info(
                "Provisioner create payload for sandbox %s: %s", sandbox_id, payload
            )
            resp = requests.post(
            ...
            data = resp.json()
            logger.info(
                "Provisioner created sandbox %s: sandbox_url=%s",
                sandbox_id,
                data["sandbox_url"],
            )
            return SandboxInfo(
    ...
    def _provisioner_destroy(self, sandbox_id: str) -> None:
        ...
        try:
            ...
            if resp.ok:
                ...
                logger.warning(
                    "Provisioner destroy returned %s: %s", resp.status_code, resp.text
                )
        except requests.RequestException as exc:
