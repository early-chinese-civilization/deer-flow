...
def test_create_skill_release_generates_release_version_and_persists_metadata():
    async def run():
        ...
        assert release.source_skill_id is None
        assert release.published_skill_id is None
...
def test_create_skill_release_rejects_missing_terminal_version():
    async def run():
        session = FakeAsyncSession(skill_version=None)
...
def test_get_latest_published_release_for_skill_returns_release():
    async def run():
        skill_id = uuid.UUID("12121212-1212-4121-8121-121212121212")
        ...
        result = await SkillReleaseRepository.get_latest_published_release_for_skill(session, skill_id=skill_id)
...
def test_get_latest_published_release_for_skill_returns_none_for_missing_skill():
    async def run():
        session = FakeExecuteSession(None)

        result = await SkillReleaseRepository.get_latest_published_release_for_skill(session, skill_id=uuid.uuid4())
...
def test_latest_published_release_by_name_uses_terminal_release_identity():
    async def run():
        ...
        assert "skill_releases.skill_version_id" not in sql
...
def test_platform_skill_version_install_models_declares_manifest_foundation_columns():
    ...
    assert "artifact_uri" not in version_columns
...
def test_install_bound_agent_lookup_includes_disabled_active_bindings():
    ...
    sql = asyncio.run(run())
    assert "agent_skills.skill_installation_id = 20" in sql
    assert "agent_skills.enabled" not in sql
