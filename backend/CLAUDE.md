...
| **Skills** (`/api/skills`) | `GET /` - list visible skills with version metadata; `GET /{name}` - details; `PUT /{name}` - refresh/update current user skill; `POST /install` - install from `.skill` archive; `POST /{name}/publish` - publish a custom skill as public latest and create a release record with optional release notes; `POST /{name}/download` - copy public latest into the current user's custom skills; `POST /check-upload` / `POST /{name}/check-download` - conflict checks. Optional SKILL.md frontmatter includes `version`, `author`, `compatibility`; non-string `version` is rejected. Upload checks compare same-name custom skills by package version: same version is duplicate, different version updates the current custom copy. |
