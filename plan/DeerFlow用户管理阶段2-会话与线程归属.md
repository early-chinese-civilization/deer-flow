...
- `threads.thread_id` 直接作为产品会话主键，同时也是 LangGraph `thread_id`
- 不新增独立业务会话 ID
- 一个 thread 最多绑定一个 agent
- 不同 thread 可以绑定同一个 agent
- `workspace_id` 在阶段 3 进入稳定绑定；v1 新 thread 默认自动分配新的隐藏 workspace

## 5. user_id 归属规则

- `threads.user_id` 一律来源于阶段 1 解析出的 `current_user`
...
- 旧 thread 采用懒接管
- 用户首次登录访问某个历史 thread 时，Gateway 补建 `threads` 归属记录
- 不做上线前全量回填
- 未被接管的匿名历史 thread 不纳入新用户隔离体系
- 被接管 thread 一旦写入 `threads`，即视为进入新用户体系
...
若阶段 2 新增 threads / messages / user_id 定向测试，验收时必须把新增测试一起纳入执行清单。

## 11. 风险与注意事项

- 若 user_id 归属规则不固定，会导致 thread 归属争议和权限判断不一致
- 若未把 `threads.thread_id = 产品 thread 主键 = LangGraph thread_id` 写成硬规则，后续若继续引入历史设想中的 `/api/chats`，会与 `/api/threads` 出现双主键漂移
- 若过早关闭 `/api/threads/**` 兼容层，会直接打挂现有前端主链路

## 12. 验收动作

- `threads.thread_id` 主键已生效
- 新体系会话消息已开始镜像写入
