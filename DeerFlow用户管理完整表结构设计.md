...
**移除的字段**:
- ❌ `skills_json` - 移到 `agents_skills` 关联表
- ❌ `memory_json` - 移到 `memories` 表
- ❌ `soul_markdown` - 移到 `memories` 表的 `soul` 字段
- ❌ `extensions_config_json` - 移到 `memories` 表的 `extensions` 字段
...
CREATE INDEX ix_skills_enabled ON skills(enabled);
CREATE INDEX ix_skills_sync_status ON skills(sync_status);
```
...
CREATE TABLE memories (
    ...
    -- Agent人格和配置（从agents移过来）
    soul TEXT,                             -- Agent人格定义（SOUL.md内容）
    extensions JSONB,                      -- Agent专属MCP配置
    
    -- 时间戳
    last_updated_at TIMESTAMP WITH TIME ZONE NOT NULL,
    created_at TIMESTAMP WITH TIME ZONE NOT NULL,
    
    CONSTRAINT uq_memories_user_agent UNIQUE (user_id, agent_id)
);

CREATE INDEX ix_memories_user_id ON memories(user_id);
CREATE INDEX ix_memories_agent_id ON memories(agent_id);
CREATE INDEX ix_memories_last_updated ON memories(last_updated_at);
```

**新增字段说明**:
- `soul`: Agent的人格定义和行为准则（原 `agents.soul_markdown`）
- `extensions`: Agent专属的MCP服务器配置（原 `agents.extensions_config_json`）
...
```sql
ALTER TABLE threads 
    ADD COLUMN agent_id BIGINT REFERENCES agents(id) ON DELETE SET NULL;
...
```
users (用户表)
  ├─→ agents (1:N)          用户的agents
  ├─→ skills (1:N)          用户的skills
  ├─→ memories (1:N)        用户的memories
  └─→ threads (1:N)              用户的threads

agents (Agent表)
  ├─→ agents_skills (1:N)         Agent绑定的skills
  ├─→ memories (1:N)        Agent专属memory
  └─→ threads (1:N)              使用该Agent的threads

skills (Skills表)
  └─→ agents_skills (1:N)         被哪些Agents使用
...
```sql
-- 1. 创建Agent
INSERT INTO agents (user_id, name, description, model)
VALUES (1, 'python-expert', 'Python专家', 'claude-sonnet-4-6')
RETURNING id;  -- 返回 agent_id = 100

-- 2. 创建Skills
INSERT INTO skills (user_id, name, description, oss_path)
VALUES 
...
-- 3. 绑定Agent和Skills
INSERT INTO agents_skills (agent_id, skill_id, display_order)
VALUES 
    (100, 200, 1),
    (100, 201, 2);

-- 4. 创建Agent专属Memory
INSERT INTO memories (user_id, agent_id, memory_json)
...
SELECT s.* 
FROM skills s
JOIN agents_skills as ON as.skill_id = s.id
WHERE as.agent_id = 100
ORDER BY as.display_order;

-- 查询Agent的Memory（包含Soul和Extensions）
SELECT memory_json, soul, extensions
FROM memories 
...
3. **灵活查询**
   ...
   JOIN agents_skills as ON as.agent_id = a.id
...
-- 2. 迁移Skills数据
INSERT INTO skills (user_id, name, oss_path, ...)
SELECT 
    user_id,
    skill_name,
...
-- 3. 迁移Agent-Skills关联
INSERT INTO agents_skills (agent_id, skill_id)
SELECT 
    a.id,
    s.id
FROM agents a
CROSS JOIN LATERAL jsonb_array_elements_text(a.skills_json) skill_name
JOIN skills s ON s.name = skill_name AND s.user_id = a.user_id;
...
-- 5. 删除旧字段
ALTER TABLE agents DROP COLUMN skills_json;
ALTER TABLE agents DROP COLUMN memory_json;
```
