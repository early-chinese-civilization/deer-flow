...
When you install `.skill` archives through the Gateway, DeerFlow accepts standard optional frontmatter metadata such as `version`, `author`, and `compatibility` instead of rejecting otherwise valid external skills. If `version` is present, it must be a string; DeerFlow treats it as a user-facing package version, not as enforced SemVer. Upload conflict checks compare the current user's same-name custom skill by package version: same name and same version is a duplicate, while same name with a different version updates that user's current custom skill copy rather than creating a second active skill.

Custom skills can be published into the public catalog as the current public latest. Each publish creates an immutable system release record with a generated `release_version`, the optional package `version` from `SKILL.md`, and optional publish-event release notes entered in the Skills Gallery. The `skills` table still represents the current visible/installable copy used by list, download, and agent binding; release records answer which publish produced a public latest copy. The Skills Gallery shows package versions when available, falls back to release versions, and requires confirmation before publishing a custom skill as public latest.
...
Gateway auth now follows the shared ECC Keycloak contract (`ecc-auth`) with JWKS-first request verification, an app-level auth-session middleware that persists rotated cookies for dependency-protected responses, and an explicit `kc_logout_marker`, so logging out does not silently restore the session through refresh fallback. The auth router is initialized after config/.env loading and now fails fast when required Keycloak env vars are missing, which keeps direct Gateway startup aligned with `make dev` and Docker behavior. The backend workspace now resolves `ecc-auth` from the sibling checkout during local development, so Gateway tests and runtime imports exercise the same SDK source that carries the shared auth-session middleware. Gateway-backed agent runs derive protected runtime context (`thread_id` and user identity) from server-side authentication and ownership checks, strip runtime-only flags like `is_bootstrap` from caller config, and only persist a sanitized public run config in run records, so client-supplied context cannot spoof the agent's user or thread binding. The frontend now routes explicit logout transitions to `/signed-out` instead of bouncing back through `/workspace`, which avoids redirect loops while the logout marker is still present. The old handwritten Gateway Keycloak helper stack has been removed from the runtime path, and thread-scoped artifact downloads, follow-up suggestions, and stateless run reuse all require authenticated ownership checks; auto-created stateless run threads are cleaned up on completion by default.
